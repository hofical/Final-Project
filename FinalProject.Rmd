...
## Testing
Note: I found it hard to come up with a test that is interesting in any way as all the variables relate to the ingredients of the food (a mistake in choice of data in my part). For instance a variable containing ratings of the items would have been very useful. 
...
The prerequisits for linear regression seem to be met.
...
```
The first model has an R2 of 97% which is very high. The result is not surprising at all. Calories depend very much on the nutrients of the food. Sugar plays a comparatively small role in determining calories which must be because of the small amount of deserts present in this dataset. 
A second model is considered where only the significant predictors are kept. 
